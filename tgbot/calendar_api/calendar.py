...
from create_bot import local_tz, calendar_service
...
async def get_events(schedule_date: Union[date, dt], start_time=time.min, end_time=time.max):
    try:
        start_time = dt.combine(
            schedule_date, start_time).astimezone(timezone(local_tz)).isoformat()
        end_time = dt.combine(
            schedule_date, end_time).astimezone(timezone(local_tz)).isoformat()
